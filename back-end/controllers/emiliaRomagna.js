...
const pagina = "emiliaRomagna",
  ...
  erroreFile =
    "Si e' verificato un errore interno al server: il file richiesto potrebbe non esistere",
...
const CountTxt = (req, res) => {
  fs.readdir(TxtDir + "ita", (err, files) => {
...
const CountDidascalia = (req, res) => {
  fs.readdir(DidascalieDir + "ita", (err, files) => {
    if (err) res.status(400).send(err);
    else {
      if (files.length) res.status(200).send({ count: files.length });
      else res.status(200).send({ count: 0 });
...
const CountDidascaliaTre = (req, res) => {
  fs.readdir(DidascalieTreDir + "ita", (err, files) => {
    if (err) res.status(400).send(err);
    else {
      if (files.length) res.status(200).send({ count: files.length });
      else res.status(200).send({ count: 0 });
...
const Txt = (req, res) => {
  ...
  if (lingua && id) {
    console.log(
      "Testo " + pagina + " con id=" + id + " e con lingua=" + lingua
    );
...
const Didascalia = (req, res) => {
  ...
  if (lingua && id) {
    console.log(
      "Didascalia " + pagina + " con id=" + id + " e con lingua=" + lingua
    );
...
const Img = (req, res) => {
  ...
  if (id) {
    ...
    imgExtensions.every((imgExtension) => {
      let tempPath = ImgDir + id + imgExtension;
